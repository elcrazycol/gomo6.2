-- 094_wall_post_views.sql
--
-- View counting for profile wall posts ("стена").
--
-- A "view" is recorded when a wall post becomes visible in the viewer's
-- viewport (the frontend observes the card and reports it in batches via
-- POST /api/rpc/record_wall_views). Counting is per UNIQUE VISITOR:
--   * authenticated viewers dedupe on viewer_id (one view per user per post);
--   * anonymous browsers dedupe on viewer_key (a persistent client-generated
--     id stored in localStorage — one view per browser per post).
--
-- The UNIQUE constraints treat NULLs as distinct, so rows with only a
-- viewer_id and rows with only a viewer_key coexist without colliding, while
-- duplicate (post_id, viewer_id) and (post_id, viewer_key) pairs are rejected.
--
-- The visible count is a correlated subquery (same pattern as likes/comments/
-- reposts in wallPostCountsSQL) — no denormalized counter column to keep in
-- sync, and the unique indexes cover the COUNT(*) lookups.

CREATE TABLE IF NOT EXISTS profile_wall_post_views (
...
-- The unified feed embeds per-post interaction counts, so wall posts need the
-- views_count there too (the feed card renders it). CREATE OR REPLACE cannot
-- change a function's RETURNS TABLE signature, so DROP + CREATE.
