-- 090_user_feed_fix.sql
--
-- Redeploys get_user_feed() with two semantic fixes. Migration 089 shipped the
-- function already; because schema_migrations tracks by FILENAME, editing
-- 089_user_feed.sql alone would never re-run on databases where 089 already
-- applied. This file re-creates the function so existing deployments pick up
-- the fix on the next backend restart.
--
-- What changed vs the 089 version:
--   1. private_profile no longer filters THREADS. A private profile gates
--      only the profile WALL — threads/posts are public content the author
--      writes on boards, so hiding them would make authorship pointless.
--      This matches GetThreads/GetThread, which never filter threads by
--      private_profile. The wall-post branch keeps the full privacy rule.
--   2. The caller's OWN threads and wall posts are included in the feed.
--      Otherwise a user whose content is all their own (typical while the
--      platform has little content) would see only other people's wall posts.
--
-- Fresh installs get the same end state via 089 (already fixed) + 090.
