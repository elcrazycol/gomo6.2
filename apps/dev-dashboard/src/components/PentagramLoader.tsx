    ...
    if (!document.getElementById("pentagram-loader-styles")) {
      ...
      style.textContent = `
        ...
        @keyframes pentagram-pulse {
          0%, 100% { filter: brightness(1) drop-shadow(0 0 15px hsl(var(--primary) / 0.8)); transform: scale(1) rotate(0deg); }
          25% { filter: brightness(1.3) drop-shadow(0 0 20px hsl(var(--primary) / 0.9)); }
          50% { filter: brightness(1.8) drop-shadow(0 0 30px hsl(var(--primary) / 1)); transform: scale(1.1) rotate(-4deg); }
          75% { filter: brightness(1.3) drop-shadow(0 0 20px hsl(var(--primary) / 0.9)); }
        }
      `;
    ...
    <div className={cn("flex items-center justify-center", className)}>
      <div className={cn("pentagram-loader", sizeClasses[size])}>
        <svg viewBox="0 0 100 100" preserveAspectRatio="xMidYMid meet" className="w-full h-full">
          <path className="pentagram-path" d="M50,95 L5,10 L95,63 L5,63 L95,10 Z" />
        </svg>
      </div>
    </div>
  );
