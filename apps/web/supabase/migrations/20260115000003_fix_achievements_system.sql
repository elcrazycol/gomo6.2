-- Fix achievements system - separate migration to apply all changes
...
AS $$
DECLARE
  base_achievement_id TEXT;
  current_level INTEGER;
BEGIN
  -- Determine base achievement ID for each type
  CASE _achievement_type
    WHEN 'time' THEN base_achievement_id := 'time_10min';
    WHEN 'posts' THEN base_achievement_id := 'posts_10';
    WHEN 'threads' THEN base_achievement_id := 'threads_5';
    WHEN 'settings' THEN base_achievement_id := 'font_customizer';
    WHEN 'activity' THEN
      -- For activity, we need to determine which one
      IF _level = 1 THEN
        -- Default to rules_reader, but this could be improved
        base_achievement_id := 'rules_reader';
      END IF;
    ELSE
      RETURN; -- Unknown type
  END CASE;
  ...
  IF NOT EXISTS (SELECT 1 FROM achievements WHERE id = base_achievement_id) THEN
    RETURN;
  END IF;
  ...
  WHERE user_id = _user_id AND achievement_id = base_achievement_id;

  IF current_level IS NULL THEN
    -- First time achievement
    INSERT INTO user_achievements (user_id, achievement_id, level)
    VALUES (_user_id, base_achievement_id, _level);
