...
const verifyOrigin = (value: string | undefined) => {
  ...
  const allowed = [process.env.APP_BASE_URL, "https://gomo6.wtf", "https://www.gomo6.wtf"].filter(Boolean);
  return allowed.some((origin) => origin === value);
};

const sanitizeCookieDomain = () => {
  const raw = process.env.SHARED_COOKIE_DOMAIN || ".gomo6.wtf";
...
export default async function handler(req: any, res: any) {
  ...
  const baseUrl = process.env.MESSENGER_BASE_URL || "https://m.gomo6.wtf";
