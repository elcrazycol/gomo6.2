...
const EmojiCreate = () => {
  ...
  const createEmoji = async () => {
    if (!selectedGroup || !emojiName.trim() || !emojiCode.trim() || !selectedFile) {
    ...
    try {
      ...
      toast.success('Эмодзи создан успешно!');
      navigate('/moderation/emojis');

    } catch (error) {
      console.error('Error creating emoji:', error);
      toast.error('Ошибка создания эмодзи');
    } finally {
      setIsCreating(false);
    }
  };
    ...
    <div className="bg-background min-h-screen">
      <header className="bg-board-header text-board-header-foreground p-3 border-b border-border">
        <div className="max-w-5xl mx-auto flex items-center justify-between gap-2">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-xl font-bold">Создание эмодзи</h1>
          </div>
      ...
      <main className="max-w-2xl mx-auto p-4">
        <div className="space-y-6">
          ...
          {selectedGroup && emojiName && emojiCode && selectedFile && (
          ...
          <Button
            onClick={createEmoji}
            disabled={isCreating || !selectedGroup || !emojiName.trim() || !emojiCode.trim() || !selectedFile}
            className="w-full"
            size="lg"
          >
            {isCreating ? 'Создание...' : 'Создать эмодзи'}
          </Button>
        </div>
      </main>
    </div>
