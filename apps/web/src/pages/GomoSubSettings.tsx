...
const GomoSubSettings = () => {
  ...
  useEffect(() => { const load = async () => { setLoading(true); const { data: { user } } = await api.auth.getUser(); if (!user) { navigate("/auth"); return; } const { data: board } = await api.from("boards").select("id,name,description,rules_markdown,gomosub_avatar_url,cover_image_url,owner_id,gomosub_tags,is_gomosub,visibility").eq("slug", slug).eq("is_gomosub", true).maybeSingle(); if (!board) { toast.error("G-саб не найден"); navigate("/g"); return; } if (board.owner_id !== user.id) { toast.error("Только создатель может менять настройки"); navigate(`/g/${slug}`); return; } const tags = Array.isArray(board.gomosub_tags) ? board.gomosub_tags.filter((t: unknown): t is string => typeof t === "string") : []; const parsedRules = parseRulesMarkdown(board.rules_markdown); setStandardRules(parsedRules.standardRules); setAllowedRules(parsedRules.allowedRules); setForbiddenRules(parsedRules.forbiddenRules); setOriginalRulesMarkdown(board.rules_markdown ?? null); setIsOwner(true); setBoardVisibility(board.visibility || "public"); setForm({ id: board.id, name: board.name || "", description: board.description || "", gomosub_avatar_url: board.gomosub_avatar_url || "", cover_image_url: board.cover_image_url || "", gomosub_tags: tags }); setLoading(false); const promises = [loadChannels(board.id), loadRoles(board.id), loadMembers(board.id)]; if (board.visibility === "private") promises.push(loadInvites(board.id)); await Promise.all(promises); }; load(); }, [navigate, slug]);
