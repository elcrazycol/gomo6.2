...
export async function loadLibsignal(): Promise<void> {
  ...
  loadPromise = (async () => {
    // Provide require() shim for libsignal-protocol (it uses require internally)
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    // Load long.js (provides window.Long for libsignal-protocol)
    await loadScript("/long.js");
    // Load libsignal-protocol (uses window.Long and require internally)
