...
## 9. Пост-деплой верификация (12.08.2026, прод gomo6.wtf)

Коммиты: `80646d11` (M1), `f28f3016` (L1/L2), `4fd17a3c` (I1) → запушены в `codeberg/main` → Codeberg Actions → Docker-образы → VPS. Проверено на проде после деплоя:

| Находка | Проверка на проде | Результат |
|---|---|---|
| M1 (now_playing WS) | Анонимная подписка на `profile_now_playing_<id>` (ws_probe.py) | Denied, fail-closed (exit 2, no_connected_confirmation) ✅ |
| L1 (search private) | Анонимный `GET /api/v1/search?type=users&q=admin` | `{"users":[]}` — private-профили не утекают ✅ |
| L1 (rate limit) | 140 уникальных запросов к `/search` параллельно | 120×200 + 20×429 — ровно лимит 120/мин ✅ |
| L2 (SW-кэш) | `sw.js` на проде: cache name + urlPattern | `storage-objects-v2` + negative lookahead `(?!uploads\/|wall\/)` ✅ |
| I1 (react-router 7) | Браузер: загрузка, навигация, поиск, консоль | Новый бандл, навигация без ошибок, только ожидаемые 401 ✅ |
| Инфраструктура | `/health`, docs/dev, HSTS/CSP | Всё OK ✅ |

Полный авторизованный тест M1 (private-профиль + не-друг vs друг) на проде не проводился — тестовые аккаунты удалены после аудита, Turnstile блокирует авторегистрацию; фикс покрыт 18 тестами локально.
