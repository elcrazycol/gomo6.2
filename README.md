...
## 🔧 Последние улучшения (июнь 2026)

### GomoSub-каналы — Discord-подобная система

- **Сайдбар каналов** — плавающая стеклянная карточка (`backdrop-blur-md`, `rounded-xl`), приклеенная к верхнему левому углу вьюпорта (`sticky top-4`). Остаётся на месте при любом скролле тредов.
- **Сворачивание/разворачивание** — кнопка в заголовке карточки, отдельная sticky-кнопка при свёрнутом состоянии.
- **Группировка по категориям** — каналы автоматически группируются, категории отображаются заголовками.
- **Приватные каналы** — ролевая модель доступа (чтение/запись) через `channel_permissions`.
- **Drag-and-drop** перестановка порядка каналов в настройках.
- **Моментальное переключение** — загрузка доски и тредов разделены: при смене канала подгружаются только треды, без полноэкранного лоадера.

### Исправление дублирования тредов при быстром переключении

- **fetchIdRef** — каждый вызов `loadThreads` получает уникальный ID. Если ответ приходит после начала нового запроса (race condition при быстром переключении каналов) — он отбрасывается.
- **Защита infinite scroll** — сброс `loadingMoreThreads(false)` в обоих guard-ах, чтобы спиннер не зависал при прерывании load-more.

### Правила гомосабов — надёжное принятие и отображение

- **Кеш принятия правил инвалидируется мгновенно** — после вызова `POST /api/v1/gomosub_rules_acceptance` сбрасываются все Redis-паттерны `user_id=eq.*` и `board_id=eq.*`. Диалог больше не появляется при каждом заходе.
- **`rules_updated_at` больше не сбрасывается при каждом save** — фронтенд хранит `originalRulesMarkdown` и отправляет `rules_markdown` в апдейте только если правила реально изменились. Бэкенд (`UpdateBoard`) сравнивает старое и новое значение перед bump-ом даты.
- **Markdown-рендеринг правил** — `bbcodePlugins.tsx` теперь распознаёт `### Заголовок` (→ `<h3>`), `## Заголовок` (→ `<h2>`), `- пункт` (→ `<li>`). Правила отображаются со стилизованными заголовками и списками вместо сырого текста.

### Кеш-инвалидация аватаров и обложек гомосабов

- **`UpdateBoard` теперь сбрасывает кеш доски** — после изменения аватара/фона/настроек вызывается `cache.InvalidateForBoard(h.redis, id, slug)`. Изменения видны сразу, а не через 5 минут.
- **Path-based инвалидация для slug/username** — `BuildCachePatterns` генерирует `data:/api/v1/boards/shroom?*` (а не только `*slug=eq.shroom*`), что покрывает кеш-ключи от эндпоинтов вида `/api/v1/boards/:slug`.
