...
Собственный видеоплеер в X/Telegram-стиле и сквозная механика «тап по видео → страница поста/треда с автоплеем» на всех поверхностях: стена профиля, фид записей, фид тредов и gomo-треды.

### Added
- **Собственный видеоплеер в X/Telegram-стиле** вместо нативного `<video>` (Plyr потерялся под CSP):
  - Кастомные контролы: play/pause, прогресс-бар с перемоткой и живым буфером, громкость/мут, полноэкранный режим через нативный Fullscreen API
  - Контролы автоматически прячутся во время воспроизведения и всплывают по касанию/ховеру
  - Полная клавиатурная карта: пробел — play/pause, стрелки — перемотка, M — мут, F — полноэкранный режим, Esc — выход
- **Тап по видео открывает страницу поста/треда с автоплеем** — как в X: вместо проигрывания в ленте клип открывается на своей странице и сразу стартует приглушённым. Работает на стене профиля, в фиде записей, в фиде тредов и в gomo-тредах
- При нескольких видео в посте автоплеем запускается только первый клип — остальные ждут тапа
- `run-local.sh` бэкенда добавляет Homebrew `bin` в PATH — `ffprobe`/`ffmpeg` резолвятся, загрузка видео перестаёт падать с «Failed to process video»

### Fixed
- **Автоплей на мобильных (iOS Safari) теперь стабильно работает**: клип создаётся и стартует приглушённым — iOS отклоняет `play()` со звуком после SPA-перехода, поэтому попытка «сначала со звуком» убрана. Это ограничение платформы: как в X/Instagram, автоплей идёт без звука, звук включается тапом
